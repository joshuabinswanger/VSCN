 ...
 */
export const onUserCreated = onDocumentCreated({ document: "users/{uid}", secrets: [githubPat] }, async () => {
    ...
    if (!res.ok) {
        console.error("Failed to trigger GitHub Actions:", res.status, await res.text());
    }
    else {
