...
import { queueSignup } from "./adminDigest";
import { purgeAccount } from "./purge";
import { dispatchRebuild } from "./rebuild";

/**
 * Puts the new account on the operator's queue (adminDigest.ts). Nothing is
 * mailed here: Auth-create is wizard step 1, before verification and before
 * the wizard is done, so the digest waits for the wizard to finish (or for
...
export const onAuthUserCreated = functionsV1.auth.user().onCreate(async (user) => {
  try {
    ...
    logger.error("Signup not queued", { uid: user.uid, err: String(err) });
  }
});
