> Mirrors the `~/.claude/projects/D--SynoDrive-VSCN/memory/member-profile-pages-live.md` memory file; keep the two in sync.
...
version of the prototype profile page — real route, real data, indexed.
...
`src/lib/membersBuild.ts` → `toMemberView()` in `src/lib/memberView.ts`. 84 pages built
then, 42 of them member pages, all in the sitemap — **the total is 62 since the prototype
...
"Röttele" → `roettele`, not `rottele`), collisions get `-2`, `-3`, and a nameless profile
...
The slug is **derived every build, never stored**, which is what makes it follow a rename —
...
of the prototype: real heading outline (`h1` name, `h2` section keys — verified), alt text
...
PhotoSwipe moved here from `MemberCard` per Josh's call — the card's arrow carousel replaces
...
dev member pages and PhotoSwipe was exercised against a real gallery — opens with
images, closes. The pages now also carry per-member OG images and the mname-/mimg-
transition names matching the directory cards (the card→page morph).
...
a real image via a dynamic import — opens, loads, closes cleanly (the close is animated;
...
`fetchMemberViews()` — `CommunityGrid`/`MemberCard` read raw fields the view model does not
...
/ keep GSAP — all since decided): [[release-b-shipped-to-dev]].

**Update 2026-08-28 — `/members` is now a page, and MemberCard is back.** Josh asked for
...
card, revived from `main` as `src/components/MemberCard.astro` — adapted to `MemberView`
...
(Firestore order), no client shuffle — an index, not a mood board. So "MemberCard goes and
embla dies with it" above is half-superseded: MemberCard is alive again, but still without
Embla — the note about `embla-carousel` being a dead dependency still holds. The same day
added the Gallery/Grid view selector on /community — that story lives in
